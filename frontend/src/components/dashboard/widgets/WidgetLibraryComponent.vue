<template >
    <div v-if="showWidgetbar" class="bg-white px-2 h-screen transition-all duration-500 flex flex-col fixed top-0 w-[400px] right-0 z-10 border-l-2 select-none">
        <!-- Header -->
        ...
        <div> 
            ...
            <input v-model="search" type="text" class="bg-gray-50 border border-gray-300 text-slate-500 text-sm rounded-lg block w-full pl-10 p-2.5 " placeholder="Search widget" required>
            </div>
        </div>
        ...
        <table class="mt-3 border-gray-600  border-separate px-4">
            <tbody v-if="filteredSearch">  
                <tr v-for="(widget, index) in filteredSearch" :key="index">
                    <td class="bg-gray-300 hover:bg-gray-400 rounded-sm px-2 py-2 cursor-pointer" @click="addWidget(widget)">{{widget.icon}} {{ widget.title }}</td>
                </tr>
            </tbody>
